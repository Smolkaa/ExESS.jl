...
function Cartesian3DGrid(T::Type{<:AbstractFloat}, xrange::Tuple, yrange::Tuple, 
    ...
    y1, y2 = T.(sort([yrange[1], yrange[2]]))
    z1, z2 = T.(sort([zrange[1], zrange[2]]))
...
function Cartesian3DGrid_Exponential(T::Type{<:AbstractFloat}, xrange::Tuple, yrange::Tuple, 
    ...
    z1, z2 = T.(sort([zrange[1], zrange[2]]))
    dx, dy, dz = x2 - x1, y2 - y1, z2 - z1
    x = T.(range(x1 + dx/N_x/2, x2 - dx/N_x/2, N_x))
    y = T.(range(y1 + dy/N_y/2, y2 - dy/N_y/2, N_y))
    z = T.(z1 .+ [dz * (exp(c/N_z)^(i-0.5) - 1) / (exp(c) - 1) for i in 1:N_z])
