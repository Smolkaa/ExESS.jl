...
influenced by graviational forces. Returns the landing position as a tuple of global 
spherical coordinates (radius, longitude, latitude). Returns `(NaN, NaN, NaN)` if the 
particle escapes.

# Arguments
- `x0::Tuple` or `x0::AbstractVector` or `x0::AbstractPosition`: Initial position in global
  spherical coordinates.
- `v0::Tuple` or `v0::AbstractVector` or `v0::AbstractVelocity`: Initial velocity in local
...
(local cartesian coordinates: x (east), y (north), z (up)). Returns the time of flight in 
seconds. Returns `NaN` if the particle escapes.
...
_Internal function, not exported to user._
"""
...
"""
    _relative_kinetic_energy(vesc, v)

Calculates the relative kinetic energy of the velocity vector `v` with respect to the escape
velocity `vesc`.

# Arguments
- `vesc::Real`: Escape velocity.
- `v::Tuple` or `v::AbstractVector` or `v::LocalCartesianVelocity`: Velocity vector in local
  cartesian coordinates.
